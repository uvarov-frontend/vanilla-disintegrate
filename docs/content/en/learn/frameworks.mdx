...
This is a new DOM node, so pass the same `effect` to `restore()` yourself if the removal did not use the instance default. The library only remembers an effect for the exact node it removed.
