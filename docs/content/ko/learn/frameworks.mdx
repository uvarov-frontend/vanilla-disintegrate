...
복원된 요소는 새 DOM 노드입니다. 인스턴스 프리셋을 상속하지만 제거 시 다른 작업 수준 `preset`이나 사용자 `effect`를 선택했다면 `restore()`에도 같은 표현을 전달하세요. 기억된 표현은 동일하게 보관된 DOM 노드에만 자동으로 따라갑니다.
...
Solid는 signal이 소유한 분기를 동기적으로 갱신합니다. `setVisible(true)` 뒤의 마이크로태스크는 새 노드가 삽입된 후 복원이 실행되도록 합니다.
