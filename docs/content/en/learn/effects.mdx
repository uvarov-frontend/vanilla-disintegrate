...
`dust` and `wind` have audio on both phases. Their restore sound is the removal file played backwards, through the `reverse` option — the motion is reversed, so the sound is too. `vapor` and `scatter` are silent on both phases; there are no files for them yet.
