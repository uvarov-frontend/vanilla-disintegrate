...
两个工厂都需要快照。如果 `AnimationContext.snapshot` 为 `null`，它们会返回 `null`：操作会尝试已配置的 `fallback`，否则跳过动画。
...
如果渲染器无法获取上下文，DOM 操作仍会提交。配置了 `fallback` 时会改为运行它；否则 `finished` 以 `status: 'skipped'` 结束，并通过 `onError` 报告失败。
