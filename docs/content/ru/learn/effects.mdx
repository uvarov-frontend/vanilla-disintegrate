...
У каждой фазы собственный слот аудио. Сейчас звук есть у `dust.remove` и `wind.remove`; остальные шесть слотов намеренно равны `null` до появления подходящих файлов. `sound: false` отключает звук, не меняя анимацию.
