...
This is a new DOM node. It inherits the instance preset; if removal selected another operation-level `preset` or custom `effect`, pass the same presentation to `restore()`. Remembered presentation follows only the exact retained DOM node.
