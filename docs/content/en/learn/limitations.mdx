...
The `capture` option accepts any adapter capable of returning a `<canvas>`, so a project **can use another engine or a custom implementation**. The [Capture API](/docs/reference/capture/) defines the complete adapter contract.
...
If WebGL2 is unavailable or the browser cannot create a context, **the DOM operation still takes place**, but without a fallback the visual effect resolves with a `skipped` status and the reason is passed to `onError`.

Built-in particles and custom phases with `requires: 'webgl2'` can receive a `fallback`. It runs when WebGL2 is unavailable or the built-in particle renderer cannot start; with unavailable WebGL2, the check happens **before creating a Canvas snapshot**. A fallback may be a CSS, WAAPI, Canvas 2D, SVG, or other custom animation, cannot require WebGL2, and intentionally runs without sound.
