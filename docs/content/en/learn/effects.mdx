...
Each phase has its own sound slot. `dust.remove` and `wind.remove` currently include audio; the other six slots intentionally remain silent until matching assets are added. `sound: false` disables audio without changing animation.
