...
Это новый DOM-узел. Он наследует пресет экземпляра; если при удалении был выбран другой `preset` операции или пользовательский `effect`, передайте ту же конфигурацию в `restore()`. Запомненная конфигурация автоматически следует только за тем же сохранённым DOM-узлом.
...
export function DisintegratingCard() {
  ...
  return visible ? (
    ...
    <button onClick={restoreCard}>Восстановить</button>
  );
}
```
...
<template>
  ...
  <button v-else @click="restoreCard">Восстановить</button>
</template>
```
...
export function DisintegratingCard() {
  ...
  return (
    <Show when={visible()} fallback={<button onClick={restoreCard}>Восстановить</button>}>
      <article ref={card}>
        <h2>Активность</h2>
        <button onClick={removeCard}>Удалить</button>
      </article>
...
@Component({
  ...
  template: `
    @if (visible()) {
      ...
      <button (click)="restoreCard()">Восстановить</button>
    }
  `,
})
