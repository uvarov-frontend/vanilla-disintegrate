...
每个阶段都有独立声音槽。`dust`、`vapor`、`scatter` 与 `wind` 的两个阶段都有音频：恢复音轨即删除音轨在播放时经 `reverse` 选项倒放，与粒子的反向运动对应。声音需显式开启：`sound: true` 为实例或单次调用启用内置音频，`sound: false` 保持关闭且不改变动画。
