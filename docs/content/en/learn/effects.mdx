...
`dust`, `scatter` and `wind` have audio on both phases. Their restore sound is the removal file played backwards, through the `reverse` option — the motion is reversed, so the sound is too. `vapor` is silent on both phases; there is no file for it yet.
