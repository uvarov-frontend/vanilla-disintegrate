...
Обе фабрики требуют снимок. Если `AnimationContext.snapshot` равен `null`, они возвращают `null`: при настроенном `fallback` операция попробует его, иначе пропустит анимацию.
...
Если рендерер не может получить контекст, операция с DOM всё равно выполняется. При настроенном `fallback` запустится он; иначе `finished` завершается со `status: 'skipped'`, а ошибка передаётся в `onError`.
