...
Visual Chromium instances use `--disable-lcd-text` so live DOM text and transparent SVG/canvas text both use grayscale antialiasing. Linux otherwise paints the live reference with LCD subpixels, producing color differences across glyphs even when layout and fonts match. This normalizes the reference renderer without changing comparison tolerances or library behavior.
